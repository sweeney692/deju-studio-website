...
- **Press On Nails Workshop flyer is built (2026-06-13).** Promotional flyer for Desty's one-day workshop (450.000 IDR), in two directions x two formats, all on the Deju brand system. Lives in `marketing/flyer/` (currently **untracked** - not yet committed). Six deliverables: Editorial + Bold, each A4 (210x297mm print PDF) and Instagram Story (1080x1920 PNG + PDF). The **Bold** direction is the iterated favourite; **Editorial** is its own bone-cream look. Reproducible build pipeline in `marketing/flyer/build/` (4 HTML templates -> `inline.py` data-URI inliner -> `build.sh` Chrome-headless export, same convention as the tabletents). Rebuild all with `cd marketing/flyer && bash build/build.sh`. Full write-up incl. print gotchas: `Summaries/2026-06-13-workshop-flyer.md`. **Three print gotchas to remember:** (1) Chrome print-to-PDF turns `box-shadow`/`text-shadow` into hard rectangles - use `filter: drop-shadow()` or solid-fill panels; (2) inverted QR codes (light-on-dark `qr-cream`) scan in the PNG but FAIL in the PDF - always use dark-on-light `qr-forest.png`; (3) always verify the actual PDF (`qlmanage -t -s 1600 -o <dir> file.pdf`), not just the on-screen render.
...
13. **Workshop flyer follow-ups (`marketing/flyer/`).** See `Summaries/2026-06-13-workshop-flyer.md` for the full picture. Open items: (a) **Bold A4 QR fix - recommended.** `build/photographic-a4.html` still uses the inverted `qr-cream.png`, which scans in the PNG but fails in the PDF print path; swap its `<img>` to `qr-forest.png` and rebuild (the other 3 deliverables already use it). Conor was asked and hadn't decided. (b) Pick a final direction (Editorial vs Bold) or keep both. (c) Get the A4s print-tested at the Bali shop (scan the QR off a draft before a full run). To rebuild any flyer: `cd marketing/flyer && bash build/build.sh`, and **always check the exported PDF** (`qlmanage -t -s 1600 -o <dir> "<file>.pdf"`), not just the on-screen render - the shadow/QR print gotchas only show up in the PDF. The whole `marketing/` tree is currently untracked; decide whether to commit it (the build pipeline + final PDFs are worth keeping; the raw product photos and the `Salinan dari MENU COURSE.pdf` draft are not).
